...
import { Siren, Plus, Search, FileDown } from 'lucide-react';
...
export default function Emergency() {
  ...
  const canWrite = [...CLINICAL, 'receptionist'].includes(user.role);
  ...
  const handleSubmit = async (values) => {
    try {
      if (editing) {
        ...
        setToast('Emergency record updated successfully');
      } else {
        await create(values);
        setToast('Emergency record added successfully');
      }
      setModalOpen(false);
    ...
    <div className="flex flex-col h-full bg-slate-50 p-6 md:p-8">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800 flex items-center gap-3">
            <div className="p-2.5 bg-rose-100 text-rose-700 rounded-xl">
              <Siren size={26} strokeWidth={2.5} />
            </div>
            Emergency Ward
          </h1>
          <p className="text-slate-500 mt-2 font-medium">Manage critical patients and immediate triage workflows.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <div className="relative">
            ...
            <input
              ...
              className="w-full sm:w-64 pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all shadow-sm"
            />
          </div>
          {canExport && (
          ...
          {canWrite && (
            <button onClick={handleAdd} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-600 text-white rounded-xl hover:bg-rose-700 transition-all shadow-md shadow-rose-500/20 font-medium">
              <Plus size={20} />
              Admit ER Patient
            </button>
          )}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm flex-1 overflow-hidden flex flex-col">
        <DataTable
          ...
          canWrite={canWrite}
          emptyLabel="No active emergency cases."
        />
      </div>

      {modalOpen && (
        <FormModal
          title={editing ? "Update Emergency Case" : "Admit ER Patient"}
          fields={fields}
          initialData={editing}
